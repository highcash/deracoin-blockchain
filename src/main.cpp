...
#if defined(NDEBUG)
# error "Deracoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0d707b5f22e771623e11066a7ce11b4cd9d80ee3d25f4b09819505f5f626365f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Deracoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Deracoin Signed Message:\n";
...
{
    // Deracoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Deracoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Deracoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Deracoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Deracoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Deracoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x25b0b44513f7b7169206275b1f4e5f263ba343d1d1d5af45372ee0eb53a8634e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "The Deracoin project was developed by the 1Devlab team alongside Ali";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040c29ed38360205c2a779a4f062e778106c6e41d8fecbb9d70e4d3669c171b18f29c67a9c0eaf0bc100e504749d1311c96d0d0cb95af16e250ecfc9c8417492c1") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1594031196;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085067137;

        if (fTestNet)
        {
            block.nTime    = 1594031184;
            block.nNonce   = 386771697;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xcaf3fa986e69698f3a5311fc869895b10737ac0b05bef68daef0449e9d406871"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc2, 0xba, 0xd2 }; // Deracoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DeracoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DeracoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DeracoinMiner(CWallet *pwallet)
{
    printf("DeracoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("deracoin-miner");
    ...
    try { loop {
        ...
        printf("Running DeracoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DeracoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DeracoinMiner, pwallet));
}
